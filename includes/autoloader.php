...
class Autoloader {
	...
	private static function load_class( $relative_class_name ) {
		...
		if ( isset( self::$classes_map[ $relative_class_name ] ) ) {
			$filename = GROUNDHOGG_WSG_PATH . '/' . self::$classes_map[ $relative_class_name ];
		} else {
			$filename = strtolower(
				preg_replace(
					[ '/([a-z])([A-Z])/', '/_/', '/\\\/' ],
			...
			$is_filename = GROUNDHOGG_WSG_PATH . $filename . '.php';

			if ( ! file_exists( $is_filename ) ){
			    $filename = wp_normalize_path( GROUNDHOGG_WSG_PATH . 'includes/' . $filename . '.php' );
            } else {
